...
const mockTasks = [
  ...
  },
  {
    ...
    project: 'e_commerce_storefront',
    type: 'Task',
    assigned_user: 'jane.smith@example.com',
    status: 'In Review',
    estimation: 2,
    slug: 'tsk-3',
...
export const seedTasks = async (
  ...
  const promises = mockTasks.map(async (task) => {
    const user = users.find((user) => user.email === task.createdBy);
    ...
    const type = statuses.find((status) => status.label === task.type);

    if (!user || !project || !status || !type) {
